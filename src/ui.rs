...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    if app.is_searching {
        ...
        let help_text = match app.mode {
            AppMode::FileSystem => {
                " q:Quit | TAB:Switch Mode | /:Search | ?:About | h:Hidden | j/k/↓/↑:Nav | Enter:Play | Bksp:Up | Space:Pause | +/-:Vol | ←/→:Track | l:Loop "
            }
            AppMode::Radio => {
                " q:Quit | TAB:Switch Mode | /:Search | ?:About | j/k/↓/↑:Nav | Enter:Play | Space:Pause | +/-:Vol "
            }
        };
        let help_paragraph =
